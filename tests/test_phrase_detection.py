...
class TestPhraseDetection:
    ...
    def test_phrase_in_sentence(self):
        ...
        result = detect_termination_phrase(
            "Well, goodbye everyone", self.termination_phrases
        )
        assert result == "goodbye"

        result = detect_termination_phrase(
            "I think that's all for now", self.termination_phrases
        )
        assert result == "that's all"

        result = detect_termination_phrase(
            "Please end call now", self.termination_phrases
        )
        assert result == "end call"
    ...
    def test_multi_word_phrases(self):
        ...
        result = detect_termination_phrase(
            "I want to end call", self.termination_phrases
        )
        assert result == "end call"

        result = detect_termination_phrase(
            "that's all I need", self.termination_phrases
        )
        assert result == "that's all"
    ...
    def test_partial_matches_with_extra_words(self):
        ...
        result = detect_termination_phrase(
            "please end the call", self.termination_phrases
        )
        assert result == "end call"

        result = detect_termination_phrase(
            "I think that's really all", self.termination_phrases
        )
        assert result == "that's all"

        result = detect_termination_phrase(
            "can you end this call", self.termination_phrases
        )
        assert result == "end call"

        result = detect_termination_phrase(
            "well that's definitely all for today", self.termination_phrases
        )
        assert result == "that's all"
    ...
    def test_exact_word_matching_required(self):
        ...
        result = detect_termination_phrase(
            "that is really all for today", self.termination_phrases
        )
        assert result is None

        # But "that's" with extra words should still match
        result = detect_termination_phrase(
            "I think that's definitely all", self.termination_phrases
        )
        assert result == "that's all"
