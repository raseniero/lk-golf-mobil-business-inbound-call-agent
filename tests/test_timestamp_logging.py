...
class TestTimestampLogging:
    ...
    async def test_call_start_timestamp_logging(self):
        ...
        with patch('agent.logger') as mock_logger:
            ...
            start_logged = any(
                'Call started at' in str(call) and 'T' in str(call)  # ISO format contains 'T'
    ...
    async def test_call_end_timestamp_logging(self):
        ...
        with patch('agent.logger') as mock_logger:
            ...
            end_logged = any(
                'Call ended at' in str(call) and 'T' in str(call)  # ISO format contains 'T'
                for call in mock_logger.info.call_args_list
            )
            assert end_logged
    ...
    async def test_call_duration_with_timestamps(self):
        ...
        with patch('agent.logger') as mock_logger:
            ...
            duration_logged = any(
                'duration' in str(call).lower() for call in mock_logger.info.call_args_list
            )
    ...
    async def test_log_call_lifecycle_timestamps(self):
        ...
        with patch('agent.logger') as mock_logger:
            ...
            start_timestamp_logged = any('started at' in log.lower() for log in all_logs)
