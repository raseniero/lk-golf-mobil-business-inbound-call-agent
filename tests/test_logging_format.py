...
class TestLoggingFormat:
    ...
    async def test_log_format_call_start(self):
        ...
        with patch('agent.logger') as mock_logger:
            ...
            self.agent._log_call_event("CALL_START", {
                "start_time": datetime.fromtimestamp(self.agent.call_session.start_time).isoformat()
            })
    ...
    async def test_log_format_phrase_detection(self):
        ...
        with patch('agent.logger') as mock_logger:
            ...
            phrase_logged = any(
                'PHRASE_DETECTED' in str(call) for call in mock_logger.info.call_args_list
            )
    ...
    async def test_log_format_call_termination(self):
        ...
        with patch('agent.logger') as mock_logger:
            ...
            termination_logged = any(
                'CALL_TERMINATION' in str(call) for call in mock_logger.info.call_args_list
            )
