...
class TestCallStateManagement:
    ...
    def agent(self):
        ...
        with patch('livekit.agents.voice.Agent.__init__', return_value=None), \
             patch('livekit.agents.voice.Agent.session', new_callable=PropertyMock) as mock_session_prop:
    ...
    async def test_call_flow_states(self, agent, caplog):
        ...
        assert any("Call ended. Duration:" in record.message for record in caplog.records)
