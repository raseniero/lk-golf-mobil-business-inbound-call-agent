...
def _get_or_create_combined_patterns(termination_phrases: Set[str]) -> Tuple[re.Pattern, Dict[str, str], re.Pattern, Dict[str, str]]:
    ...
    if phrases_key not in _COMBINED_PATTERN_CACHE:
        ...
        if single_word_phrases:
            escaped_singles = [re.escape(phrase.lower()) for phrase in single_word_phrases]
        ...
        _COMBINED_PATTERN_CACHE[phrases_key] = (single_pattern, single_map, multi_pattern, multi_map)
...
def detect_termination_phrase(text: str, termination_phrases: Set[str]) -> Optional[str]:
    ...
    single_pattern, single_map, _, multi_map = _get_or_create_combined_patterns(termination_phrases)
